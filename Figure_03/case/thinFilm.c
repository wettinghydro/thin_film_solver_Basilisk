...
$$ h_t + \mathbf\nabla \cdot \left[h^3 \mathbf\nabla\Delta h
- h^3 \left( \underbrace{\tilde\Pi^\prime(\mathbf x, h)}_
{\text{substrate}\atop\text{heterogeneities}}
+ \underbrace{w_1}_{\text{body forces}} \right)\mathbf\nabla h\right]
+ \underbrace{\nabla_\mathrm{x_1} \left(h^3w_2\right)}_{\text{body forces}}
...
=\frac{\mathcal E -\delta(\Delta h-\Pi(h) {- w_1 h})}{\mathcal K+h} $$
...
$$ h_t + \mathbf\nabla \cdot \left[h^3 \mathbf\nabla
\Delta {\color{crimson} z} - h^3 \tilde\Pi^\prime(\mathbf x, h)
\mathbf\nabla h
- h^3 w_1 \mathbf\nabla {\color{crimson}z} \right]
+ \nabla_\mathrm{x_1} \left(h^3w_2\right)
+ \frac{\mathcal E -\delta(\Delta {\color{crimson} z} -\Pi(h)-w_1{\color{crimson} z})}
{\mathcal K+{\color{crimson} z}} =0$$
...
*/
#if INCLINED
double bond = 3.75, inclineA_d = 40., contactA_d = 45.0;
#endif
...
are provided.
*/
const double tolMG = 1.e-4, tolNewton = 1.e-10;
...
*/
const int printStatsEvery = 100, fieldSolToSave = 20, noOfFigsToSave= 100;
char dirFigures[]    = "png/";
...
int LEVEL_IN  = 4; // initial resolution (optional input arg)
int LEVEL_MAX = 4; // maximum resolution
...
double pressureTermByParts (const double hf)
{
...
double ham_dpdh (const double hetf, const double hf)
{
	return hetf * dpdh(hf);
}

double ham_dpdh_der (const double hetf, const double hf)
{
	return hetf * dpdh_der(hf);
}

double ham_pressureTermByParts (const double hetf, const double hf)
{
	return hetf * pressureTermByParts(hf);
}
...
*/
		...
		foreach_face()
		{
			...
			A.x[]  = cube(hf);
			B.x[]  = A.x[] * ham_dpdh(hetf, hf);

			C.x[]  = ham_pressureTermByParts(hetf, hf) * gradH.x[];
...
*/
event outputContactLines(t+=Tend/noOfFigsToSave, last)
{
...
event movie(t+=Tend/noOfFigsToSave, last)
{
...
{
	{
		char b[100]; sprintf(b, "%sslice_x0_%06.02f.gnp", dirGnu, t);
...
event outputSolution (t+=Tend/fieldSolToSave, last)
{

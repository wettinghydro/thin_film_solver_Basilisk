...
double precFilm = 3.e-3, n = 3.0, m = 2.0;
...
*/
#if INCLINED
double bond = 15.00, inclineA_d = 0., contactA_d = 45.0;
#endif
...
*/
const int printStatsEvery = 10, fieldSolToSave = 150, noOfFigsToSave= 250;
char dirFigures[]    = "png/";
...
$$
*/
double het_disPressureDer(double hetf, double hf, double h_grad)
{
	return pressureDerTermByParts(hf) * hetf * h_grad
		    + pressureByParts(hf, h_grad);
}
...
*/
		struct ContactLineInitializer
					cli1 = {.r = 1.0, .a=4.0, .c = {.x = 0.0, .y = 0.0} };
...
*/
		...
		heterogeneity(false,
		 (2.0 + 0.1*x - 0.2*(tanh(cos(pi*(x+2.0*y))) + tanh(cos(pi*(2*x-y)))))
			 );
...
/**
Write the initial contact lines
*/
	{
		char fname[80]; sprintf(fname, "%spostlines%d.gnp", dirGnu, i);
...
*/
event outputContactLines(t=0.00001; t*=1.2, last)
{
	char fname[80]; sprintf(fname, "%spostlines%06.04f.gnp", dirGnu, t);
...
*/
#if 0
event slicer(t=0.000001; t*=1.05, last)
{
	{
...
event outputSolution (t=0.0001; t*=1.2, last)
{

...
struct ContactLineInitializer cliFile;
/**
...
void simpleParapoloidInitialization (FILE* fp, struct ContactLineInitializer cl)
{
	if (!cl.r) return;
	...
	foreach()
	{
		...
		if (r2Cell < r2)
			h[] = max(precFilm, cl.a * (1.0 - (r2Cell)*r2Inv));
	}
...
//precursor film
void smoothParapoloidInitialization (FILE *fp, struct ContactLineInitializer cl)
{
	if (!cl.r) return;
	if (!fp) fp = fopen("circles.dat", "w");

...
// do more general
void readDropletInitialization(char *inName, struct ContactLineInitializer *cli)
{
	FILE* fp = fopen(inName, "r");
...
void printDropletParameters()
{
	fprintf(fout, "#DropletInitialization:\n");
	fprintf(fout, "#a r cx cy\n");
	fprintf(fout, "%g %g %g %g\n",
			cliFile.a, cliFile.r, cliFile.c.x, cliFile.c.y);
	fprintf(fout, "\n");
}
...
void setupFromFile(int *argc, char *argv[])
{
	for (int i = 0; i < *argc; i++)
	{
		...
		if (!strcmp (&argv[i][len-4], ".dat") )
		{
			...
			readDropletInitialization(argv[1], &cliFile);

			printDropletParameters();

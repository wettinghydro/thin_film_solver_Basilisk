/**
# Utilities for setting up cases via file. Needs refactoring.
 */
...
// do more general
ContactLineInitializer *inputDroplets(char *inName)
{
	FILE* fp = fopen(inName, "r");
	...
	Array *dropsIn = array_new();

	int toSkip = 1;
	while (getline(&b, &bsize, fp) != -1)
	{
		if (strstr(b, "Droplet") != NULL ) toSkip = 0;

		if (toSkip || b[0] == '#') continue; //skip comments

		ContactLineInitializer cli;

		int reads = sscanf(b, "%lf %lf %lf %lf",
				&cli.a, &cli.r, &cli.c.x, &cli.c.y);

		if (reads != 4) continue;

		array_append(dropsIn, &cli, sizeof(ContactLineInitializer));
	}
	...
	return (ContactLineInitializer *) array_shrink(dropsIn);
}

void printDropletParameters(ContactLineInitializer * cl)
{
	fprintf(fout, "#DropletInitialization:\n");
	fprintf(fout, "#a r cx cy\n");

	int i = 0;
	while (cl[i].a != nodata)
	{
		fprintf(fout, "# %g %g %g %g\n",
				cl[i].a, cl[i].r, cl[i].c.x, cl[i].c.y);
		i++;
	}

	fprintf(fout, "#Found %d drops. \n", i);
	fprintf(fout, "\n");
}
...
void setupFromFile(int *argc, char *argv[])
{
	for (int i = 0; i < *argc; i++)
	{
		...
		if (!strcmp (&argv[i][len-4], ".dat") )
		{
			...
			dropsList = inputDroplets(argv[1]);
